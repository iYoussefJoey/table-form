...
export class TableComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit(): void {
    ...
    if (cols) {
      ...
      this.displayedColumns = ['fullName','dOB', 'gender', 'gradutionYear', 'phone', 'email', 'lastModificationDate', 'studentParent', 'actions'];  
    }
  ...
  loadData() {
    ...
    if (Array.isArray(students)) { // chech if the data is an array
      this.dataSource.data = students.map(student => ({...student, fullName: this.getFullName(student)} )) // Create a new reference
